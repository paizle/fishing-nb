 ...
 *
 *   public/data/index.json            -> [{ id, name, nameField, nid, lat, lng, area }] sorted by name
 *   public/data/geometry/{cell}.json  -> [{ id, name, nameField, nid, lat, lng, geometry }] per grid cell
...
import { getNameSourceFromProps, getNidFromProps } from './waterNameUtils.mjs';
...
function main() {
  ...
  for (const feature of features) {
    ...
    const nameSource = getNameSourceFromProps(props);
    const name = nameSource?.value ?? '';
    const nameField = nameSource?.field ?? '';
    const rawNid = props.NID != null ? String(props.NID).trim() : '';
    const nid = getNidFromProps(props);
    if (rawNid.startsWith('{')) nidGuidFormat++;
    if (rawNid && !nid) nidInvalid++;
    ...
    index.push({ id, name, nameField, nid, lat, lng, area });
    ...
    cells.get(key).push({ id, name, nameField, nid, lat, lng, geometry });
  }
  ...
  if (nidGuidFormat) {
    console.log(
      `Note: ${nidGuidFormat} features have GUID-form NID values in source ` +
        `(e.g. "{xxxxxxxx-xxxx-...}") — stored as-is from the NID property.`
    );
  }
  if (nidInvalid) {
    console.log(`Warning: ${nidInvalid} features had invalid NID values (discarded).`);
  }
}
