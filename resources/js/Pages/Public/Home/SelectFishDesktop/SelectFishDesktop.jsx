...
export default function SelectFishDesktop({
	...
	const scrollFishesLeft = () => {
		if (fishListRef.current.scrollLeft === 0) {	
			...
			fishListRef.current.scrollLeft = fishListRef.current.scrollLeft - fishListRef.current.querySelector('[data-id]:first-child').getBoundingClientRect().width
		}	
	}

	const scrollFishesRight = () => {
		if (fishListRef.current.scrollLeft + fishListRef.current.clientWidth >= fishListRef.current.scrollWidth) {	
