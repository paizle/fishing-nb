...
VerifySourceModal (React)
  ...
  → iframe src=/regulations/Fish.pdf#page=N  (desktop)
  → PDF.js canvas render (browsers in VerifySourcePdfViewer::PDF_JS_USER_AGENT_PATTERNS)
```
...
| [`app/Support/VerifySourcePdfViewer.php`](../../app/Support/VerifySourcePdfViewer.php)                     | User-Agent list for PDF.js fallback — add patterns here     |
| [`resources/views/verify-source.blade.php`](../../views/verify-source.blade.php)                           | iframe or PDF.js branch                                     |
| [`scripts/copy-pdfjs.mjs`](../../scripts/copy-pdfjs.mjs)                                                   | Copies `pdfjs-dist` into `public/pdfjs/` on `npm run build` |
...
**PDF.js (`pdfjs-dist`, devDependency):** approved for verify-source on browsers that cannot inline PDF in iframes. Deploy must include `public/pdfjs/` (generated by `npm run build`).
