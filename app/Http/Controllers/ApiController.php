...
{
	private function createLastModified(
		$year,
		$month,
		$day,
		$hour = 0,
		...
		foreach ($limits as $limit) {
			...
			if ($water !== '') {
				...
				if ($limits_by_location[$key] ?? null === null) {
					$limits_by_location[$key] = [
						'locationId' => $limit->location_id,
						'waterId' => $limit->water_id,
					];
				}
			} else {
				if ($limits_by_location[$key] ?? null === null) {
		...
		foreach ($limits->toArray() as $limit) {
			...
			if ($water_id !== '0') {
				if ($limit['water_id'] === null) {
					if (
						$limit['waters_category_id'] !== null &&
						$limit['waters_category_id'] !== $water_category_id
					) {
						$add = false;
					}
				} elseif ($limit['water_id'] != $water_id) {
					$add = false;
				}
			}
