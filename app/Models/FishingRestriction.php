...
class FishingRestriction extends Model
{
	...
	public static function getOrInitialize(
		FishingRestriction $fishingRestriction
	) {
		...
		if (!$record) {
			...
			$record
				->fish_category()
				->associate($fishingRestriction->fish_category);
			$record->fish()->associate($fishingRestriction->fish);
			$record
				->boundary_category()
				->associate($fishingRestriction->boundary_category);
			$record
				->waters_category()
				->associate($fishingRestriction->waters_category);
	...
	public function boundary_category()
	{
		return $this->belongsTo(BoundaryCategory::class);
	}
